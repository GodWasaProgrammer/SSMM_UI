...
def chrome_timestamp_to_datetime(ts):
    # Chrome timestamp är mikrosekunder sedan 1601-01-01 UTC
    if ts == 0:
        return None
    epoch_start = datetime.datetime(1601, 1, 1, tzinfo=datetime.timezone.utc)
    dt = epoch_start + datetime.timedelta(microseconds=ts)
    # Returnera snygg ISO-format utan microseconds, med Z för UTC
...
def main():
    ...
    # fetch all v20 cookies with expiry
    con = sqlite3.connect(pathlib.Path(cookie_db_path).as_uri() + "?mode=ro", uri=True)
    cur = con.cursor()
    r = cur.execute("SELECT host_key, name, CAST(encrypted_value AS BLOB), expires_utc FROM cookies;")
    ...
    for c in cookies_v20:
        ...
        name = c[1]
        expiry_raw = c[3]
        expiry = chrome_timestamp_to_datetime(expiry_raw)
    ...
    # Spara till fil
    out_path = os.path.join(os.getcwd(), "cookies_v20.json")
    with open(out_path, "w", encoding="utf-8") as f:
        f.write(json_data)
    print(f"\nCookies sparade till fil: {out_path}")
